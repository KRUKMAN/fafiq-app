...
- Timeline: `ActivityTimeline` -> `useDogTimeline(orgId, dogId)` -> `fetchDogTimeline(orgId, dogId)` -> RPC `get_dog_timeline(p_org_id, p_dog_id)`
...
- Row click / "History": opens transport detail drawer (no dedicated timeline yet)
...
- Upload document: currently uploads a sample text blob -> storage upload + `createDocumentRecord` (transport entity)
- Open document: signed URL
- Delete document: `deleteDocumentRecord(orgId, documentId)` (admin per RLS) with inline confirm (TODO: invalidate `['documents', orgId, 'transport', transportId]` after changes)
