-- Use constraint name to avoid email ambiguity in org_invites upsert.
...
begin
  ...
  if found and v_user_id is not null then
    ...
    set status = 'added_existing',
        ...
        accepted_at = now(),
        updated_at = now()
    where oi.org_id = p_org_id and lower(oi.email) = v_email;
  ...
  on conflict on constraint org_invites_org_id_email_key do update
